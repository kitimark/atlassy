...
Production source files (`src/lib.rs`, `src/main.rs`) SHALL NOT contain `mod tests { ... }` blocks with test function bodies. Production files MAY contain only thin test module declarations: `#[cfg(test)] mod tests;` or `#[cfg(test)] mod test_helpers;`.

#### Scenario: Production file with thin declaration only
- **WHEN** a production `src/` file references a test module
- **THEN** it contains only `#[cfg(test)] mod tests;` (a declaration, not a block with braces and body)

#### Scenario: Grep verification finds no inline test blocks
...
Tests that call private (non-`pub`) functions, methods, or types SHALL be placed in dedicated test files under `src/` (e.g., `src/tests.rs`) within the same crate. The test file uses `use super::*;` to access the parent module's private items.
...
#### Scenario: Confluence tests access private build methods
- **WHEN** `atlassy-confluence` tests call private `LiveConfluenceClient::build_publish_payload` or `build_create_payload`
- **THEN** those tests reside in `crates/atlassy-confluence/src/tests.rs`
