...
- Migrate release workflow execution to `release-plz/action@v0.5` and pin related actions by version tag (`actions/checkout@v6`, `actions/create-github-app-token@v3`) to reduce per-run setup time while keeping updates deliberate.
...
- Workflow implementation: release pipeline uses version-tag pinned actions (`@v6`, `@v3`, `@v0.5`) and should be updated intentionally during dependency maintenance.
